...
public class USSS4 {
    ...
    public void monitorStart(SensorMonitor sensorMonitor) throws InterruptedException {
        ...
        System.out.println("Sensor 4 - Waiting for the sensor to be ready (2s)...");
        Thread.sleep(2_000L);
        ...
        while (go) {
            ...
            try {
                synchronized (mainThread) {
                    ...
                    if (DEBUG) {
                        System.out.println("Sensor 4 - MainThread done waiting (" + diff + " ms)");
                    }
                    if (diff >= MAX_WAIT) {
                        ok = false;
                        if (true || DEBUG) System.out.println("Sensor 4 - ...Reseting...");
                        if (trigger.isAlive()) {
                            trigger.interrupt();
                        }
                    }
            ...
            if (ok) {
                ...
                if (DEBUG) {
                    System.out.println("Sensor 4 - Measuring...");
                }
                if (end > 0 && start > 0) {
                    ...
                    if (distance > MIN_DIST && distance < MAX_DIST) {
                        if (!sensorMonitor.isBlocked() && sensorMonitor.isActiveSensor4()) {
                        ...
                        try {
                            ...
                            if (!sensorMonitor.isBlocked()) {
                                sensorMonitor.setBlocked(true);
                                sensorMonitor.setWhoBlocked(4);
                                sensorMonitor.setSensorOn4(false);
                            }
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
